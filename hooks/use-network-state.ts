...
// Mock NetInfo para desarrollo - reemplazar con la implementación real
const NetInfo = {
  fetch: async () => ({
    isConnected: true,
...
export function useNetworkState(options: UseNetworkStateOptions = {}) {
  ...
  const testEndpointFn = useCallback(async (url?: string): Promise<boolean> => {
  ...
  // Función para verificar la fuerza de la señal (simulada para demo)
  const checkSignalStrength = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      console.log(`📶 [Network] Signal strength check: ${hasStrongSignal} (${responseTime}ms)`);
  ...
  const forceReconnect = useCallback(async (): Promise<void> => {
    ...
    try {
      console.log(`🌐 [Network] Attempting reconnect (attempt ${networkState.reconnectAttempts + 1}/${maxReconnectAttempts})`);
      ...
      if (networkState.reconnectAttempts + 1 < maxReconnectAttempts && enableAutoReconnect) {
        console.log(`🌐 [Network] Scheduling next reconnect in ${reconnectInterval}ms`);
  ...
  const handleNetworkStateChange = useCallback(async (state: NetInfoState) => {
    ...
    setNetworkState(prev => ({
      ...
      isOnline,
      hasStrongSignal,
      lastConnectedAt: isOnline && !prev.isOnline ? now : prev.lastConnectedAt,
      lastDisconnectedAt: !isOnline && prev.isOnline ? now : prev.lastDisconnectedAt,
    }));
  ...
  }, [handleNetworkStateChange, enableSignalStrength, checkSignalStrength, networkState.isConnected]);
  ...
  useEffect(() => {
    const unsubscribe = NetInfo.addEventListener(async state => {
      ...
      if (isConnected && wasOffline) {
        console.log('🌐 [Recovery] Connection restored, executing recovery callback');
        try {
          await callback();
        } catch (error) {
